...
import { authResponseSchema, calendarQuerySchema } from "./lib/validators";
...
app.get("/refresh", authMiddleware, async (c) => {
  ...
  if (user) {
    ...
    await Promise.all([
      ...
      ...[-1, 0, 1].map((offset) =>
        c.env.CACHE.delete(
          `events:${user.id}:${new Date().getFullYear() + offset}`,
        ),
      ),
    ]);
  }
...
app.get("/", authMiddleware, async (c) => {
  ...
  const hiddenHashes = new Set(hide ? hide.split(",") : []);

  // Parse hidden event hashes from URL
  const hiddenEventHashes = new Set(hideEvents ? hideEvents.split(",") : []);
  ...
  const visibleEvents = allEvents.filter((e) => {
    ...
    if (
      e.calendarId !== BIRTHDAYS_CALENDAR_ID &&
      birthdayKeys.has(`${e.summary.toLowerCase()}|${e.start}`)
    ) {
      return false;
    }
    return true;
